...
if os_name.startswith("Linux"):
    ...
    # Fix for torsocks syscall issue 217
    os.system("git clone https://git.torproject.org/torsocks.git")
    os.chdir("torsocks")
    ...
    c1 = input(
        "Enter y if you don't have python installed, n if you have it already : ")
    if c1 == "y":
        print("Installing python 3.9.6.....")
        wget.download(
            "https://www.python.org/ftp/python/3.9.6/python-3.9.6-amd64.exe", bar=wget.bar_adaptive)
        filename = wget.detect_filename(
        ...
        print("Installing java 17.....")
        wget.download(
            "https://download.bell-sw.com/java/17.0.3+7/bellsoft-jdk17.0.3+7-windows-amd64.msi", bar=wget.bar_adaptive)
        filename = wget.detect_filename(
            "https://download.bell-sw.com/java/16.0.2+7/bellsoft-jdk16.0.2+7-windows-amd64.msi")
    ...
    os.system(
        f"shortcut.exe /F:Pycraft.lnk /A:C /T:{currn_dir}\\pycraft_gui.py /W:{currn_dir}")
    os.system("cls")
