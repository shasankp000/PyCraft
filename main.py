...
# Sets the theme of the comboboxes and progressbar. Cosmo is a light-blue theme
style = Style(theme="flatly")
style.configure("TNotebook.Tab", foreground="#15d38f",
                background="#23272a", bordercolor="#072A6C")
...
uid = data["User-info"]["UUID"]
accessToken = data["accessToken"]
mc_dir = data["Minecraft-home"]
auth_type = data["User-info"]["AUTH_TYPE"]
...
allocated_ram_selected = data["setting-info"]["allocated_ram_selected"]
...
if os.path.exists(f"{currn_dir}/.minecraft"):
...
class Pycraft():
    ...
    def __init__(self):
        ...
        self.os_name = data["PC-info"]["OS"]
        '''self.username = data["User-info"]["username"]
        self.uid = data["User-info"]["UUID"]
        ...
        if os_name.startswith("Windows"):
            self.window.iconbitmap(f"{currn_dir}/icon.ico")
        ...
        if not connected:
            ...
            self.versions_folder = f"{mc_dir}/versions"

            self.options_dl = ("Vanilla", "Forge", "Fabric", "Ares Client")

            self.sub_folders = [self.name for self.name in os.listdir(
                self.versions_folder) if os.path.isdir(os.path.join(self.versions_folder, self.name))]
            # prints list of minecraft versions
            self.offline_versions = list(self.sub_folders)
            ...
            self.vanilla_version_list = minecraft_launcher_lib.utils.get_available_versions(
                mc_dir)
            ...
            self.forge_versions = list(self.forge_version_list)

            self.versions = [(i["type"], i["id"])
                             for i in self.vanilla_version_list]

            self.fabric_versions.extend(iter(self.fabric_version_list))

            self.fpsversions = ["1.8.9"]
            ...
            self.download_options = Combobox(
                self.p1, textvariable=self.selected_download, width=15)
        ...
        if connected:
            ...
            self.l3.config(text=data["User-info"]["username"],
                           font=self.custom_font3, foreground="black", background="#C9CDEC")

            self.custom_font1 = Font(family="Galiver Sans", size=14)

            self.acc_method = data["User-info"]["AUTH_TYPE"]

            if self.acc_method == "mojang login":
                self.l4.config(text="mojang account", font=self.custom_font1,
                               foreground="black", background="#C9CDEC")
            elif self.acc_method == "ely_by login":
                self.l4.config(text="ely.by account", font=self.custom_font1,
                               foreground="black", background="#C9CDEC")
            elif self.acc_method == "cracked login":
                self.l4.config(text="no account", font=self.custom_font1,
                               foreground="black", background="#C9CDEC")
            ...
            self.l4.config(text="User is offline", font=self.custom_font1,
                           foreground="black", background="#C9CDEC")
        ...
        self.str_ram = data["PC-info"]["Total-Ram"].strip("    GB")
        ...
        if allocated_ram:
            if allocated_ram_selected is None:
                ...
                self.current_value_label.config(
                    text=f"Ram Assigned: {allocated_ram}", font=self.custom_font4)
        ...
        if ramlimiterExceptionBypassedSelected:
            self.cb1 = StringVar(value="selected")
        else:
            self.cb1 = StringVar(value="deselected")

        self.sn1 = Checkbutton(self.window_t, bootstyle="success-square-toggle",
                               onvalue="selected", offvalue="deselected", command=self.check2, variable=self.cb1)
    ...
    def generate_cracked_uid(self):
        ...
        if data["User-info"]["UUID"] is None:
    ...
    def printProgressBar(self, iteration, total, prefix='', suffix='', decimals=1, length=100, fill='█', printEnd="\r"):
        ...
        """
        self.percent = (f"{100 * (iteration / float(total)):.1f}")
        self.filledLength = int(length * iteration // total)
        self.bar = fill * self.filledLength + \
            '-' * (length - self.filledLength)
        print(f'\r{prefix} |{self.bar}| {self.percent}% {suffix}', end=printEnd)
    ...
    '''def check2(self):
        ...
        data["setting-info"]["fps_boost_selected"] = fps_boost_selected
    ...
    def check3(self):
        ...
        data["setting-info"]["tor_enabled_selected"] = tor_enabled_selected
    ...
    def get_current_value(self):
        return f" {self.current_value.get():.2f} MB"
    ...
    def slider_changed(self, event):
        ...
        with contextlib.suppress(NameError):
            ...
            # print(s1)
            data["setting-info"]["allocated_ram_selected"] = self.flt_s1
    ...
    def save_ram(self):
        ...
        data["setting-info"]["allocated_ram_selected"] = self.flt_s2
        ...
        if self.flt_s2 > self.med_ram:
            if ramlimiterExceptionBypassed:
                self.slider.set(self.flt_s2)
                self.current_value_label.config(
                    text=f"Ram Assigned: {self.flt_s2} MB", font=self.custom_font4)
                showinfo(title="Done",
                         message=f"Allocated {self.flt_s2} MB of ram")
                ...
                self.current_value_label.config(
                    text=f"Ram Assigned: {self.med_ram} MB", font=self.custom_font4)
                ...
                showerror(
                    title="Error!", message="Cannot assign more than 50 percent of host OS's ram. This is intended for low end pc(s) to run smoothly.")
            ...
            data["allocated_ram"] = self.flt_s2
            data["setting-info"]["allocated_ram_selected"] = self.flt_s2
            print(f"Changed to: {self.flt_s2}")
    ...
    def run_mc(self):
        ...
        self.login_method = data["User-info"]["AUTH_TYPE"]
        ...
        self.allocated_ram_selected = self.data1["setting-info"]["allocated_ram_selected"]
        ...
        with open("settings.json", "w") as js_set:
            json.dump(data, js_set, indent=4)
        if connected:
            # Checking for selected version before running minecraft.
            if self.runtime_ver.startswith("Vanilla"):
                if self.login_method == "mojang login":
                    try:
                        self.usr = data["User-info"]["username"]
                        self.pwd = self.pwd1
                        self.mc_ver = data["selected-version"].strip(
                            "Vanilla: ")
                        self.detected_ver = ""
                        ...
                        if connected:
                            if self.mc_ver.startswith("release"):
                                self.detected_ver = self.mc_ver.strip(
                                    "release ")
                        ...
                        self.login_data = minecraft_launcher_lib.account.login_user(
                            self.usr, self.pwd)
                        ...
                        data["User-info"]["UUID"] = self.options["uuid"]

                        with open("settings.json", "w") as js_set:
                            json.dump(data, js_set, indent=4)

                        self.window.withdraw()
                        self.minecraft_command = minecraft_launcher_lib.command.get_minecraft_command(
                            self.detected_ver, self.mc_dir, self.options)
                ...
                if self.login_method == "mojang login":
                    try:
                        ...
                        self.detected_ver1 = self.mc_ver[:7] + \
                            "forge-"+self.mc_ver[7:]
                        ...
                        self.minecraft_command = minecraft_launcher_lib.command.get_minecraft_command(
                            self.detected_ver1, self.mc_dir, self.options)
                        print(f"Launching minecraft version {self.mc_ver}")
                        subprocess.call(self.minecraft_command)
                ...
                if self.login_method == "mojang login":
                    try:
                        ...
                        self.minecraft_command = minecraft_launcher_lib.command.get_minecraft_command(
                            self.detected_ver2, self.mc_dir, self.options)
                        print(f"Launching minecraft version {self.mc_ver}")
                        subprocess.call(self.minecraft_command)

                    except minecraft_launcher_lib.exceptions.VersionNotFound as e:
                        showerror(
                            title="Error!", message=f"{e}. Try re-downloading this fabric version, seems like a new loader version has been released.")
                        print(e)
                    ...
                    try:
                        self.detected_ver2 = f"fabric-loader-{self.lv}-{self.mc_ver[:6]}" if connected else self.mc_ver
    ...
    def ely_authenticate(self):
        ...
        self.r = requests.get(
            f"https://authserver.ely.by/api/users/profiles/minecraft/{self.usr}")
        if self.r.status_code == 200:
            print("[OK] [200]", "User found, getting details........")
            self.r1 = requests.post(
                f"https://authserver.ely.by/auth/authenticate", data=self.acc_data)
            if self.r1.status_code == 200:
                ...
                showerror(
                    title="Error", message=f"Data entered is either incomplete or account is secured with Oauth2. Error code: {self.r1.status_code}")
                print(
                    "Data entered is either incomplete or account is secured with Oauth2")
        elif self.r.status_code == 404:
            print("[ERROR] 404", "User does not exist.")
            showerror(title="User not found",
                      message=f"The specified user does not exist. Error code: {self.r.status_code}")
    ...
    def download(self):
        ...
        if self.dl_opt == "Vanilla":
            ...
            showinfo(title="Installation started...",
                     message=f"Installing minecraft version {self.selected_ver}")
            minecraft_launcher_lib.install.install_minecraft_version(
                self.detected_ver1, self.mc_dir, callback=self.callback)

            data["selected-version"] = "Vanilla:" + \
                " " + f"{self.selected_ver}"
            with open("settings.json", "w") as f:
                json.dump(data, f, indent=4)
            ...
            self.l5.config(text=self.selected_version,
                           font=self.custom_font, fg="#15d38f", bg="#23272a")
            ...
            if supports_automatic_install(self.selected_ver):
                showinfo(title="Installation started..",
                         message=f"Installing forge version {self.selected_ver}")
                install_forge_version(
                    self.selected_ver, self.mc_dir, callback=self.callback)

                data["selected-version"] = "Forge:" + \
                    " " + f"{self.selected_ver}"
                with open("settings.json", "w") as f:
                    json.dump(data, f, indent=4)
            ...
            showinfo(title="Installation started..",
                     message=f"Installing fabric version {self.selected_ver}")
            install_fabric(self.selected_ver, self.mc_dir,
            ...
            try:
                showinfo(title="Installation started",
                         message="Installing Ares Client....")
                ...
                wget.download(
                    "https://download2390.mediafire.com/gdkggxf25fkg/92l0eryou1seqi6/AresNovemberUpdate.zip", bar=wget.bar_adaptive)
                print("Installing Ares Client.......")
                self.filename = wget.detect_filename(
                    "download2390.mediafire.com/gdkggxf25fkg/92l0eryou1seqi6/AresNovemberUpdate.zip")
                with ZipFile(self.filename, "r") as self.f1:
                    self.f1.extractall()
                move(f"{self.mc_dir}/Ares", f"{self.mc_dir}/versions")
                print("Done")
                os.remove(self.filename)

            except:
                showerror(title="Error",
                          message="Errors encountered while downloading....")
    ...
    def save_version(self):
        if connected:
            ...
            if self.dl_opt == "Vanilla":
                self.selected_ver = self.versionsList.get()
                data["selected-version"] = f"Vanilla: {self.selected_ver}"

                # self.l5.config(text=self.selected_version, font=self.custom_font, fg="#15d38f", bg="#23272a")

            elif self.dl_opt == "Forge":
                self.selected_ver = self.fversionsList.get()
                data["selected-version"] = f"Forge: {self.selected_ver}"

            elif self.dl_opt == "Fabric":
                self.selected_ver = self.frversionsList.get()
                data["selected-version"] = f"Fabric: {self.selected_ver}"

            elif self.dl_opt == "Ares Client":
                self.selected_ver = self.fpsversionsList.get()
                data["selected-version"] = f"Ares: {self.selected_ver}"
            ...
            if self.dl_opt in ["Vanilla", "Forge", "Fabric", "Ares Client"]:
                version = f"{self.dl_opt}: {self.selected_ver}"
                if self.dl_opt == "Ares Client":
                    version = "Ares: 1.8.9"
                self.selected_version = data["selected-version"]

            else:
                data["selected-version"] = self.selected_version
    ...
    def save_acc(self):
        ...
        if connected:
            ...
            if data["User-info"]["username"] is None:
                self.l3.config(text=self.u1, font=self.custom_font3,
                               foreground="#15d38f", background="#23272a")

                data["User-info"]["username"] = self.u1
                data["User-info"]["AUTH_TYPE"] = self.acc_method
                ...
                self.l3.config(text=data["User-info"]["username"],
                               font=self.custom_font3, foreground="#15d38f", background="#23272a")
            ...
            self.l3.config(text=data["User-info"]["username"],
                           font=self.custom_font3, foreground="#15d38f", background="#23272a")
            self.l4.config(text=text, font=self.custom_font1,
                           foreground="#15d38f", background="#23272a")

        else:
            data["User-info"]["AUTH_TYPE"] = "cracked login"
    ...
    def profile_window(self):
        ...
        self.options = ("mojang login", "cracked login",
                        "ely_by login") if connected else ("cracked login")
        ...
        self.b11 = Button(self.p2, text="Save",
                          command=self.save_acc, bootstyle="success-outline")
    ...
    def password_window(self):
        ...
        self.l6 = tk.Label(self.canvas4, text="Please enter your password to play. It won't be saved.", font=Font(
            family="Galiver Sans", size=16), foreground="#15d38f", background="#23272a")
        self.l6.place(x=20, y=135)

        self.b12 = Button(self.canvas4, text="Confirm",
                          command=self.confirm, bootstyle="warning-outline")
    ...
    def start_download(self):
        ...
        def close():
            ...
            res = askquestion(
                title='Abort?', message="Really cancel the download?")
            if res == "yes":
                try:
        ...
        self.l2.config(text=SpeedTracker().get_download_speed(),
                       background="#3a3a3a", foreground="#00FFFF")
        self.l2.update()
        self.l2.place(x=580, y=570)

        self.pb = Progressbar(
            self.pw, value=0, style='success.Horizontal.TProgressbar', length=500, mode="indeterminate")
        ...
        if q1 == "yes":
            ...
            try:
                ...
                self.dl_thread = Thread(
                    target=self.download)  # Download thread
